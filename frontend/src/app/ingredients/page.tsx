...
export default function IngredientsPage() {
  ...
  useEffect(() => {
    // If no ingredients are selected, all ingredients are available
    ...
    // Get names of selected ingredients
    ...
    const recipes = dbRecipes.filter((recipe) =>
      selectedIngredientNames.every((name) => recipe.ingredients.includes(name)),
    )
    setMatchingRecipes(recipes)

    // Find all unique ingredients from matching recipes
    const availableIngredientNames = new Set<string>()
    recipes.forEach((recipe) => {
      recipe.ingredients.forEach((ing) => availableIngredientNames.add(ing))
    })

    // Convert ingredient names to IDs
    const availableIds = ingredients.filter((ing) => availableIngredientNames.has(ing.name)).map((ing) => ing.id)
        ...
        <div className="flex flex-1 flex-col overflow-hidden pt-16 lg:pt-0">
          ...
          <header className="flex h-16 items-center justify-between border-b border-gray-700 bg-gray-800 px-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button className="rounded-md bg-emerald-600 p-1 text-white hover:bg-emerald-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  ...
                  className="h-4 w-4"
                >
                  <polygon points="22 3 2 3 22 21 12 13 2 3 22 21 12 13 22 3" />
                </svg>
              </button>
            </div>
          </header>
